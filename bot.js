...
bot.getMe().then(botInfo => {
  console.log(`@${botInfo.username} Connected`);
})
...
bot.on('message', async (msg) => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      bot.sendPhoto(chatId, thumbnailUrl, { caption: messageText, ...options, reply_to_message_id: msg.message_id });
    } else {
      bot.sendMessage(chatId, '❌ <b>Error fetching data from Terabox API</b>', { parse_mode: 'HTML', reply_to_message_id: msg.message_id });
    }
  } catch (e) {
    bot.sendMessage(chatId, `❌ <b>Error: ${e.message}</b>`, { parse_mode: 'HTML', reply_to_message_id: msg.message_id });
  }
});
